...
class btnkm:
    ...
    def train(
        ...
        c0 = shape_parameter_lambda_R if shape_parameter_lambda_R is not None else np.ones(max_iter)
        c_N = c0
        d0 = scale_parameter_lambda_R if scale_parameter_lambda_R is not None else np.ones(max_iter)
        d_N = d0
        lambda_R = (c0/d0)
    
        g0_m = shape_parameter_lambda_M if shape_parameter_lambda_M is not None else np.ones(I) 
        g_N = [g0_m for _ in range(D)]
        h0_m = scale_parameter_lambda_M if scale_parameter_lambda_M is not None else np.ones(I)
        ...
        for d in range(len(W_D)):
            hadamard_product_V = hadamard_product_V*temp(Phi=Phi[d], V=WSigma_D[d], R=R) # Element-wise multiplication
        ...
        for it in pbar:
            ...
            for d in range(D): # update the posterior q(vec(W^d)):
                ...
                hadamard_product_V = safe_division(hadamard_product_V, (temp(Phi=Phi[d], V=WSigma_D[d], R=R)))
                hadamard_product_mean = safe_division(hadamard_product_mean, ((Phi[d] @ W_D[d])))

                W_K_PROD_V = dotkron(Phi[d], Phi[d]).T @ hadamard_product_V #G^{d}G^{d}T
                cc, cy = dotkronX(Phi[d], hadamard_product_mean, Y)
                V_temp = np.reshape(np.transpose(np.reshape(W_K_PROD_V, (I, I, R, R), order = 'F'), axes=(0, 2, 1, 3)),(I * R, I * R),order='F')
                
                A = tau * (cc + V_temp) + np.kron(lambda_R * np.eye(R), lambda_M[d] * np.eye(I))
                try:
                ...
                hadamard_product_V = hadamard_product_V * temp(Phi=Phi[d], V=WSigma_D[d], R=R)
                hadamard_product_mean = hadamard_product_mean * (Phi[d] @ W_D[d])
            ...
            if lambda_M_update:
                for d in range(D):
                    ...
                    vtemp = np.diag(np.reshape(WSigma_D[d].reshape(I, R, I, R).transpose(0, 2, 1, 3).reshape(I**2, R**2) @(lambda_R * np.eye(R)).ravel(order='F'), (I,I)))
                    g_N[d] = (g0_m + R/2)
            ...
            if lambda_R_update:
                ...
                for d in range(D): 
                    np.transpose(np.reshape(WSigma_D[d], (I,R,I,R), order='F'),  axes=(0, 2, 1, 3))
                    mtemp = np.diag(W_D[d].T @ (lambda_M[d] * np.eye(I)) @ W_D[d])
                    vtemp = np.diag(np.reshape(
                        (lambda_M[d]*np.eye(I)).ravel(order='F').T @ WSigma_D[d].reshape(I, R, I, R).transpose(0, 2, 1, 3).reshape(I**2, R**2),
                        (R,R)))
                    d_N +=  mtemp + vtemp
            ...
            ss_error = np.dot((Y - np.sum(hadamard_product_mean, axis=1)), (Y - np.sum(hadamard_product_mean, axis=1)))
           ...
           #LOWER BOUND
            ...
            for d in range(D):
                temp22 += np.kron(lambda_R * np.eye(R),lambda_M[d] * np.eye(I)) @ (np.outer(W_D[d].ravel(order = 'F'),W_D[d].ravel(order = 'F'))  + WSigma_D[d])
            temp2 = -0.5 * np.trace(temp22)

            temp3 =  (0.5 *np.sum([safelog(np.linalg.slogdet(W)) for W in WSigma_D])) 
            
            temp4 =  np.sum(np.multiply(c_N,(1 - safelog(d_N) - (d0/d_N)))) # + np.sum(gammaln(c_N))
            
            temp5 = sum(
                np.sum(
                    gammaln(np.array(g_N[d])) + np.array(g_N[d]) * (1 - safelog(np.array(h_N[d])) - (h0_m / np.array(h_N[d])))
                    )
                ...
                if prune_rank:
                    ...
                    if R != np.max(rankest):
                        ...
                        for d in range(D):
                            ...
                            ranges = np.r_[[np.arange(I * false_indices[i], I * false_indices[i] + I) for i in range(len(false_indices))]]
                            WSigma_D[d] = np.delete(WSigma_D[d], ranges, axis=1)
                            WSigma_D[d] = np.delete(WSigma_D[d], ranges, axis=0)
                        
                        hadamard_product_mean = np.delete(hadamard_product_mean, false_indices, axis = 1)
                            
                        row_indices = {(r-1) * R + j for r in false_indices+1 for j in range(1, R+1)}
                        col_indices = {(i-1) * R + c for c in false_indices+1 for i in range(1, R+1)}
                        removed = sorted(row_indices | col_indices) 
                        removed = np.array(sorted(row_indices | col_indices)) - 1
                        hadamard_product_V = np.delete(hadamard_product_V, removed, axis = 1)
                        c0 = c0[:rankest]
                        d0 = d0[:rankest]       
        ...
        M = np.array([
        np.sum(
            (np.diag(W_D[d] @ W_D[d].T) / np.sum(np.diag(W_D[d] @ W_D[d].T))) * 100 >= 0.25
    ...
    #Store the estimated parameters for predictionen         
       ...
       # PLOTS
        if plot_results:
            ...
            axs[1].scatter(np.arange(1, it + 1), LB[:it], color='b', alpha=0.6, label='Lower Bound')
    ...
    def predict(self,
        ...
        W_D_PROD = np.ones((Phi[0].shape[0], self.W_D[0].shape[1]))  # Initialize product matrix
        for d in range(len(self.W_D)):
            W_D_PROD = np.multiply(W_D_PROD, (Phi[d] @ self.W_D[d]))  # Element-wise multiplication
