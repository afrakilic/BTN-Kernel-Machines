...
indices = [np.sort(np.random.choice(Imax, np.random.randint(1, Imax), replace=False)) for _ in range(D)]
Ws = [1e+1 * np.random.randn(len(idx), Rtrue) for idx in indices]
#  features
X = np.random.rand(N, D)
Phi = pure_power_features_full(X, Imax)

y = np.sum((Phi[0][:, indices[0]] @ Ws[0])*
           (Phi[1][:, indices[1]] @ Ws[1]) *
           (Phi[2][:, indices[2]] @ Ws[2]), axis = 1)
y = y + e
...
for d in range(len(W_D)):
    hadamard_product_V = hadamard_product_V*temp(Phi=Phi[d], V=WSigma_D[d], R=Rmax) # Element-wise multiplication
...
for it in range(100):
    ...
    for d in range(D): # update the posterior q(vec(W^d)):
        ...
        hadamard_product_V = safe_division(hadamard_product_V, (temp(Phi=Phi[d], V=WSigma_D[d], R=Rmax)))
        ...
        V_temp = np.reshape(np.transpose(np.reshape(W_K_PROD_V, (Imax, Imax, Rmax, Rmax), order = 'F'), axes=(0, 2, 1, 3)),(Imax * Rmax, Imax * Rmax),order='F')
        
        WSigma_D[d] = np.linalg.inv(tau * (cc + V_temp) + np.kron(lambda_R * np.eye(Rmax), lambda_M[d] * np.eye(Imax))) 
        W_D[d] = np.reshape((tau * WSigma_D[d] @ cy ), (Imax, Rmax), order = 'F')

        hadamard_product_V = hadamard_product_V * temp(Phi=Phi[d], V=WSigma_D[d], R=Rmax)
        hadamard_product_mean = hadamard_product_mean * (Phi[d] @ W_D[d])
    ...
    if lambda_M_update:
        for d in range(D):
            ...
            vtemp = np.diag(np.reshape(WSigma_D[d].reshape(Imax, Rmax, Imax, Rmax).transpose(0, 2, 1, 3).reshape(Imax**2, Rmax**2) @(lambda_R * np.eye(Rmax)).ravel(order='F'), (Imax,Imax)))
            g_N[d] = (g0 + Rmax/2)
    ...
    if lambda_R_update:
        ...
        for d in range(D): 
            np.transpose(np.reshape(WSigma_D[d], (Imax,Rmax,Imax,Rmax), order='F'),  axes=(0, 2, 1, 3))
            mtemp = np.diag(W_D[d].T @ (lambda_M[d] * np.eye(Imax)) @ W_D[d])
            vtemp = np.diag(np.reshape(
                (lambda_M[d]*np.eye(Imax)).ravel(order='F').T @ WSigma_D[d].reshape(Imax, Rmax, Imax, Rmax).transpose(0, 2, 1, 3).reshape(Imax**2, Rmax**2),
                (Rmax,Rmax)))
            d_N +=  mtemp + vtemp
...
for i, ax in enumerate(axes):
    col_labels = [
        f"${format_lambda(val)}$"
        for j, val in enumerate(lambda_R)
    ]

    row_labels = [
        f"${format_lambda(val)}$"
        for k, val in enumerate(lambda_M[i])
    ]
